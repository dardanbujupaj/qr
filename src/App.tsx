      ...
      <main className="space-y-8">
        <form onChange={(e) => handleChange(new FormData(e.currentTarget))}>
          ...
          <textarea
            id="content"
            name="content"
            className="field-sizing-content resize-none block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-cyan-600 sm:text-sm/6"
          ></textarea>
        </form>
        {output && (
          <>
            ...
            <pre className="bg-cyan-100 p-4 rounded-xl">
              {JSON.stringify(
                {
